    ...
    <div style={{ display: 'flex', gap: 'var(--space-lg)', height: '100%', position: 'relative' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-lg)', flexShrink: 0 }}>
          <div>
            ...
            <p className="subtitle">Historically completed and delivered tasks ({archivedTasks.length})</p>
          </div>
        </div>
            ...
            <table style={{ minWidth: '1200px', borderCollapse: 'separate', borderSpacing: 0 }}>
              <thead style={{ position: 'sticky', top: 0, background: 'var(--bg)', zIndex: 10, boxShadow: '0 1px 0 var(--border)' }}>
                <tr>
                  ...
                  <th style={{ width: '100px' }}>Delivered Date</th>
                  <th style={{ width: '100px', textAlign: 'center' }}>FTR/OTD</th>
                  ...
                  return rows.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {row.isFirstRow && (
                           <>
                             <td rowSpan={row.rowSpan} style={{ verticalAlign: 'top', fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center' }}>
                                {t.deliveredDate || '-'}
                             </td>
                             <td rowSpan={row.rowSpan} style={{ verticalAlign: 'top', textAlign: 'center' }}>
                               {t.ftrOtd ? (
                                 <span className={`badge ${t.ftrOtd === 'OK FTR' ? 'badge-success' : 'badge-danger'}`} style={{ whiteSpace: 'nowrap' }}>
                                   {t.ftrOtd}
                                 </span>
                               ) : <span style={{ color: 'var(--text-muted)' }}>-</span>}
                             </td>
                             <td rowSpan={row.rowSpan} style={{ verticalAlign: 'top' }}>
                               {reviews.find(r => r.sno === t.sno)?.reviewer || <span style={{ color: 'var(--text-muted)' }}>Unassigned</span>}
                             </td>
                           </>
                        )}
      ...
      {selectedTask && (
        <div className="card" style={{ width: '350px', flexShrink: 0, display: 'flex', flexDirection: 'column', height: '100%', overflowY: 'auto' }}>
          ...
          {selectedTask.ftrOtd && (
            <span className={`badge ${selectedTask.ftrOtd === 'OK FTR' ? 'badge-success' : 'badge-danger'}`} style={{ alignSelf: 'flex-start', marginBottom: '16px', marginLeft: '8px' }}>
               {selectedTask.ftrOtd}
            </span>
          )}
