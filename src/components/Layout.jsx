...
import { 
  ...
  Users, Archive, Bot, X, Sparkles, AlertTriangle 
} from 'lucide-react';
...
const Layout = () => {
  ...
  return (
    <div style={{ display: 'flex', width: '100vw', minHeight: '100vh', position: 'relative' }}>
      <aside style={{ width: 'var(--sidebar-w)', background: 'white', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: 'var(--space-lg) 0', zIndex: 10 }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '4px', padding: '0 var(--space-md)' }}>
          ...
          <NavItem to="/tasks" icon={<CheckSquare size={20} />} label="Task Info" />
          <NavItem to="/reviews" icon={<ClipboardCheck size={20} />} label="Task Review" />
          <NavItem to="/dsr" icon={<FileText size={20} />} label="DSR Generator" />
          <NavItem to="/team" icon={<Users size={20} />} label="Team Data Hub" />
          <NavItem to="/archive" icon={<Archive size={20} />} label="Archive" />
        </nav>
      </aside>

      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        <header style={{ height: '64px', background: 'white', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', padding: '0 var(--space-xl)', flexShrink: 0 }}>
          <h2 style={{ fontSize: '0.875rem', fontWeight: '500', color: 'var(--text-muted)' }}> Engineering Operations </h2>
        </header>
      ...
      {aiOpen && (
        <div style={{ position: 'fixed', top: 0, right: 0, width: '400px', height: '100vh', background: 'white', boxShadow: '-4px 0 24px rgba(0,0,0,0.1)', zIndex: 100, display: 'flex', flexDirection: 'column', borderLeft: '1px solid var(--border)' }}>
          ...
          <div style={{ padding: 'var(--space-xl)', flex: 1, overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-md)' }}>
              <button className="btn btn-secondary" style={{ justifyContent: 'center', padding: '12px' }} onClick={() => { setAiOpen(false); navigate('/dsr'); }}>
